...
router.get('/:apiKeyId', async (req: any, res) => {
  try {
      ...
      if (response.ok) {
        ...
        if (data && data.data && Array.isArray(data.data)) {
          return data.data.map((model: any) => {
            return {
              id: model.id,
              name: model.id.split('-').map((part: string) => part.charAt(0).toUpperCase() + part.slice(1)).join(' '),
              vendor: 'grok',
              capabilities: ['Text generation'],
...
function getPrettyModelName(modelId: string): string {
